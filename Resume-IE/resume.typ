...
- *工业工程：*掌握方法研究和作业测定的基本方法,熟悉IE七大手法及系统工程的分析方法。
- *建模仿真：*了解CAD、Plant Simulation、Anylogic等仿真工具，有动态产能模拟经验。
- *数据分析：*熟悉常用数据分析与可视化工具，如SQL、Python、Tableau，了解Pytorch深度学习框架。
- *数字化开发：*熟练使用 .Net/Java/SpringBoot/MySQL/Redis/HTML+CSS+JavaScript等进行信息系统开发，了解OPC UA、MQTT等常见通讯协议，有多段完整数字化系统设计、开发、实施和维护项目经历。
// - *数字化精益：*了解业界最新数字化精益方法论和工具，以及 IPD 等先进理念与数字化落地流程。
- *项目管理：*了解 PMO 的运作流程，有制造执行系统项目的管理工具优化及知识沉淀经验，熟悉常见项目管理工具，如 Gattan Chart、飞书多维表格、XMind等。
...
#item(
  ...
  [ *项目经理* ],
  "2024 年 04 月 – 2024 年 08 月 "
)


#tech[ Spring Boot + MySQL + Redis + MyBatisPlus + Nacos + Gateway + Nginx  + Vue3 + ElementPlus + ECharts ]

*项目简介：*基于江淮汽车发动机分公司 1.5TGDI 转产柔性装配线开发了 FAS 系统，主要实现了生产计划的接收与下发、工艺管理、质量管理、关重件管理、Andon管理、报表与生产展示看板等功能。

*调研与管理：*前期进行业务调研，负责需求文档与技术方案的撰写与评审；后期负责项目进度的跟踪与风险管理。

*程序开发：*负责生产计划、工艺管理、报表与生产展示看板等模块的的开发。
...
*功能开发：*在业务蓝图的范围内，基于.NET MVC框架，利用 OPC UA 、MQTT 等物联网协议与 PLC及能源采集客户端进行采集与交互，开发了 MES 上层管理信息系统与底层采集程序。
...
== #faBuildingColumns 科研经历
#item(
  [*基于DRL求解动态柔性作业车间调度问题*],
  [ *研究课题* ],
  "2024 年 03 月 – 2024 年 08 月 "
)

*课题简介：*考虑柔性作业车间扰动因素，通过深度强化学习实现车间的自适应动态调度。

*具体工作：* 使用Pytorch深度学习框架训练模型，利用实例和模拟生成的数据集进行模型训练与验证，对比分析了所提方法与GA、启发式规则等传统调度算法的效果，评估了其在不同扰动条件下的性能表现。

*课题收获：*了解使用Pytorch进行模型训练的基本流程，以及学术界对车间调度问题的传统与前沿解决策略。
